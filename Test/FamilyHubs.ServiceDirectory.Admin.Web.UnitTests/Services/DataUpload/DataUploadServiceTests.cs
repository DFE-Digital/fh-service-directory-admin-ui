...
public class DataUploadServiceTests
{
    ...
    public async Task UploadToApi_To_UpdateExistingOrganisation_UpdateServiceCalled()
    {
        ...
        var mockExcelReader = GetMockExcelReader(dataTable);
        ServiceDto actualServiceDto = null!;
        _mockServiceDirectoryClient.Setup(m => m.UpdateService(It.IsAny<ServiceDto>(), It.IsAny<CancellationToken>())).Callback((ServiceDto p, CancellationToken ct) => actualServiceDto = p);
    ...
    private Mock<IServiceDirectoryClient> GetMockOrganisationAdminClientService()
    {
        ...
        mock.Setup(m => m.UpdateService(It.IsAny<ServiceDto>(), It.IsAny<CancellationToken>())).Callback((ServiceDto service, CancellationToken cancellationToken) =>
        {
